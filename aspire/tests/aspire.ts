...
import { Aspire } from "../target/types/aspire";
import {
  Keypair,
  LAMPORTS_PER_SOL,
  PublicKey,
...
describe("aspire", () => {
  ...
  const program = anchor.workspace.Aspire as Program<Aspire>;
  ...
  const accounts = {
    ...
    mintDebt: mintA.publicKey,
    mintFund: mintB.publicKey,
    makerAtaDebt: makerAtaA,
    makerAtaFund: makerAtaB,
    funderAtaDebt:takerAtaA,
    funderAtaFund:takerAtaB,
    escrow,
    vault,
    tokenProgram,
  }
  ...
  it("Make", async () => {
    await program.methods
      .initialize(seed, new BN(0), new BN(1e6))
      .accounts({ ...accounts })
      .signers([maker])
      .rpc()
      .then(confirm)
